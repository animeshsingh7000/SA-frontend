...
export default function NeighbourhoodListDropDown(props: FormControlProps) {
  ...
  useEffect(() => {
    if (!effectRan.current) {
      const fetchOptions = async () => {
        try {
          const response = await getNeighbourhood();
          let data = response.data.neighbourhoods.map((item: any) => ({ 
            label: item.name,
            value: item._id,
          }));
