...
import { getLeaseStatus, getLeaseTemplate } from "../../api/admin/lease";
...
export default function LeaseStatusList(props: FormControlProps) {
  ...
  useEffect(() => {
    if (!effectRan.current) {
      const fetchOptions = async () => {
        try {
          const response = await getLeaseStatus();
