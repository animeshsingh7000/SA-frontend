...
import { getTransactionCreditType, getTransactionType } from "../../api/admin/lease";
...
export default function TransactionCreditType(props: FormControlProps) {
  ...
  useEffect(() => {
    if (!effectRan.current) {
      const fetchOptions = async () => {
        try {
          const response = await getTransactionCreditType();
