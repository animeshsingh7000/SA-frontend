...
import PerDiemModal from "../../components/Modal/PerDiemModel";


export default function PerDiemListing() {
  const { pathname } = useLocation();
  const [currentItems, setCurrentItems] = useState([]);
  const [loader, setLoader] = useState(false);
  const navigate = useNavigate();
  const [perDiemList, setPerDiemList] = useState(false);
  ...
  function navigateToEdit() {
    navigate(ROUTE_NAVIGATION_PATH.EDIT_DYNAMIC_PRINCING + '?fromPerDiem=' + 1);
  }

  const updateListItem = (editData: any) => {
    localStorage.setItem("isPerDiemPasswordProvided", 'yes');
    setCurrentItems(editData);
  }
  ...
  return (
    <>
      ...
      <PerDiemModal
        show={perDiemList}
        handleClose={() => setPerDiemList(false)}
